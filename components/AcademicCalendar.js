...
export default function AcademicCalendar({ request }) {
  ...
  const addException = (kind) => {
    ...
    if (kind === 'excluded') {
      const dateValue = excludedInputRef.current?.value || generationForm.excluded_input;
      if (!dateValue) return;
      setGenerationForm((current) => ({ ...current, [key]: [...current[key], dateValue], excluded_input: '' }));
      if (excludedInputRef.current) excludedInputRef.current.value = '';
    } else {
      const dateValue = holidayInputRef.current?.value || generationForm.holiday_input;
      const reasonValue = holidayReasonRef.current?.value ?? generationForm.holiday_reason;
      if (!dateValue) return;
      setGenerationForm((current) => ({ ...current, [key]: [...current[key], { date: dateValue, reason: reasonValue }], holiday_input: '', holiday_reason: '' }));
  ...
  return (
    <div className="space-y-5">
      ...
      <Dialog open={Boolean(termDialog)} onOpenChange={(open) => !open && setTermDialog(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{termDialog?.entity ? 'Edit Term' : 'Create Term'}</DialogTitle>
          </DialogHeader>
          <form ref={termFormRef} onSubmit={saveTerm} className="grid gap-3">
            <div>
            ...
            <div>
              ...
              <Input
                ...
                type="number"
                min="1"
                value={termForm.display_order ?? 1}
                onChange={(event) => setTermForm({ ...termForm, display_order: Number(event.target.value) })}
              />
            </div>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={saving}>Save Term</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
