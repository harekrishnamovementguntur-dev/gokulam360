...
export default function AcademicCalendar({ request }) {
  ...
  const saveTerm = async (event) => {
    ...
    try {
      const formElement = event?.currentTarget || termFormRef.current;
      ...
      await request(path, { method: termDialog.entity ? 'PUT' : 'POST', body: JSON.stringify(payload) });
  ...
  return (
    <div className="space-y-5">
      ...
      <Dialog open={Boolean(termDialog)} onOpenChange={(open) => !open && setTermDialog(null)}><DialogContent><DialogHeader><DialogTitle>{termDialog?.entity ? 'Edit Term' : 'Create Term'}</DialogTitle></DialogHeader><form ref={termFormRef} onSubmit={(event) => { event.preventDefault(); saveTerm(); }} className="grid gap-3"><div><Label htmlFor="term-name">Term name</Label><Input name="name" id="term-name" value={termForm.name || ''} onChange={(event) => setTermForm({ ...termForm, name: event.target.value })} /></div><div><Label htmlFor="term-order">Display order</Label><Input name="display_order" id="term-order" type="number" min="1" value={termForm.display_order ?? 1} onChange={(event) => setTermForm({ ...termForm, display_order: Number(event.target.value) })} /></div><div className="grid grid-cols-2 gap-3"><div><Label htmlFor="term-start">Start date</Label><Input name="start_date" id="term-start" ref={termStartRef} type="text" inputMode="numeric" placeholder="YYYY-MM-DD" pattern="\\d{4}-\\d{2}-\\d{2}" value={termForm.start_date || ''} onInput={(event) => setTermForm((current) => ({ ...current, start_date: event.currentTarget.value }))} onChange={(event) => setTermForm((current) => ({ ...current, start_date: event.currentTarget.value }))} /></div><div><Label htmlFor="term-end">End date</Label><Input name="end_date" id="term-end" ref={termEndRef} type="text" inputMode="numeric" placeholder="YYYY-MM-DD" pattern="\\d{4}-\\d{2}-\\d{2}" value={termForm.end_date || ''} onInput={(event) => setTermForm((current) => ({ ...current, end_date: event.currentTarget.value }))} onChange={(event) => setTermForm((current) => ({ ...current, end_date: event.currentTarget.value }))} /></div></div></div><DialogFooter><Button variant="outline" onClick={() => setTermDialog(null)}>Cancel</Button><Button type="button" onClick={() => saveTerm()} disabled={saving}>Save Term</Button></DialogFooter></form></DialogContent></Dialog>
