...
export function transitionSession(session, { status: nextStatus, actorId, now, details = {} }) {
  ...
  if (session.status === nextStatus) throw new AcademicCalendarError('Status is unchanged');
  return { ...session, status: nextStatus, source: 'manual', updated_at: now, change_history: [...(session.change_history || []), history('status_changed', actorId, now, { from_status: session.status, to_status: nextStatus, ...details })] };
}
