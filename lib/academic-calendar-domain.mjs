...
export function generationCandidates(term, input, existingSessions = []) {
  ...
  const existingByGenerationKey = new Map(existingSessions.filter((session) => session.generation_key).map((session) => [session.generation_key, session]));
  const dates = [];
  ...
  return {
    ...
    candidates: dates.map((sessionDate, index) => {
      const found = existingByDate.get(sessionDate) || existingByGenerationKey.get(term.id + ':' + sessionDate);
      return found ? { date: found.date, requested_date: sessionDate, action: 'preserve', session_number: found.session_number, status: found.status, source: found.source } : { date: sessionDate, requested_date: sessionDate, action: 'create', session_number: maxNumber + index + 1, status: 'scheduled', source: 'generated' };
    }),
    preserved: existingSessions.map((session) => ({ date: session.date, requested_date: session.generation_key?.startsWith(term.id + ':') ? session.generation_key.slice(term.id.length + 1) : session.date, session_number: session.session_number, status: session.status, source: session.source })),
