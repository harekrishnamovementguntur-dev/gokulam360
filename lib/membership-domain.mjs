...
export function assertMembershipStatus(status) {
  if (!MEMBERSHIP_STATUSES.includes(status)) {
    throw new MembershipDomainError(`Invalid membership status: ${status}`);
  }
  return status;
}
...
export function assertMembershipTransition(fromStatus, toStatus) {
  ...
  if (!TRANSITIONS[fromStatus].has(toStatus)) {
    throw new MembershipDomainError(`Membership cannot transition from ${fromStatus} to ${toStatus}`);
  }
}
...
export function createMembership({ id, organizationId, studentId, programId, status = 'pending', actorId, now, notes = '', metadata = {} }) {
  ...
  if (!id || !organizationId || !studentId || !programId || !actorId || !now) {
    throw new MembershipDomainError('Membership id, organization, student, program, actor, and timestamp are required');
  }
  return {
    id,
...
export function transitionMembership(membership, { status, actorId, now, reason = '' }) {
  if (!membership) throw new MembershipDomainError('Membership is required');
  if (!actorId || !now) throw new MembershipDomainError('Actor and timestamp are required');
  assertMembershipTransition(membership.status, status);

  const { current_marker: ignoredCurrentMarker, ...withoutCurrentMarker } = membership;
  const historyEntry = {
  ...
  return {
    ...withoutCurrentMarker,
    ...(status === 'archived' ? {} : { current_marker: true }),
    status,
    updated_at: now,
