...
function Classes() {
  ...
  const programs = items.filter(p => !p.parent_program_id);
  const batchesFor = (programId) => items.filter(p => p.parent_program_id === programId);
  const enrolledCount = (p) => students.filter(s => (s.program_ids || [s.program_id]).includes(p.id)).length;

  const save = async () => {
    if (!form.name.trim()) return toast.error(`${parentProgram ? 'Batch' : 'Program'} name is required`);
    if (parentProgram && !form.days_of_week.length) return toast.error('Select at least one session day');
    try {
      const payload = {
        ...form,
  ...
  return (
    <div className="space-y-5">
      <PageHeader title="Programs" subtitle="Create programs, add batches, and schedule their sessions" icon={School}
              ...
              <motion.section key={p.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="rounded-3xl glass p-5 md:p-6">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="flex items-start gap-3 min-w-0">
                    <div className="w-12 h-12 rounded-2xl bg-saffron-gradient grid place-items-center shadow shrink-0"><BookOpen className="text-white" size={20} /></div>
                  ...
                  <div className="flex gap-1 shrink-0">
                    <Button size="sm" variant="outline" onClick={() => openNewBatch(p)}><Plus size={13} className="mr-1" /> Add Batch</Button>
                    <Button size="sm" variant="ghost" onClick={() => openEdit(p)}><Edit3 size={13} className="mr-1" /> Edit</Button>
                    <Button size="icon" variant="ghost" onClick={() => del(p)}><Trash2 size={13} /></Button>
                  </div>
                </div>

                {batches.length === 0 ? (
                  <div className="mt-5 rounded-2xl border border-dashed p-5 text-center text-sm text-muted-foreground">No batches yet. Add the first batch to start scheduling sessions.</div>
                ) : (
                  <div className="mt-5 grid md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {batches.map(batch => {
                      ...
                      return <div key={batch.id} className="rounded-2xl bg-white/50 dark:bg-black/10 border p-4">
                        ...
                        <div className="mt-3 grid grid-cols-2 md:grid-cols-4 text-center gap-2"><div><div className="font-bold">{enrolled}</div><div className="text-[10px] text-muted-foreground">Enrolled</div></div><div><div className="font-bold">{batch.capacity || 0}</div><div className="text-[10px] text-muted-foreground">Capacity</div></div><div><div className="font-bold">{scheduledCount}</div><div className="text-[10px] text-muted-foreground">Scheduled</div></div><div><div className="font-bold text-primary">{remainingCount}</div><div className="text-[10px] text-muted-foreground">Remaining</div></div></div>
      ...
      <SessionScheduler batch={schedulerBatch} onClose={() => setSchedulerBatch(null)} />
    </div>
  );
}
...
function Attendance() {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-2xl glass p-4">
        {!program ? <EmptyState text="Select a batch to see enrolled students" /> :
