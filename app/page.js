...
function Students({ students, setStudents }) {
  ...
  const printCard = async () => {
    ...
    doc.setFillColor(234, 88, 12); doc.rect(0, 0, 54, 16, 'F');
    ...
    doc.setDrawColor(234, 88, 12); doc.setLineWidth(0.4); doc.roundedRect(19, 20, 16, 20, 1, 1);
    if (cardOf.photo_url) { try { doc.addImage(cardOf.photo_url, 'JPEG', 19.2, 20.2, 15.6, 19.6); } catch { } }
    else { doc.setFontSize(6); doc.setTextColor(150); doc.text('PHOTO', 27, 30.5, { align: 'center' }); }
    doc.setTextColor(30); doc.setFontSize(7);
    let y = 45;
    const line = (label, value) => { doc.setFont('helvetica', 'bold'); doc.text(label, 4, y); doc.setFont('helvetica', 'normal'); doc.text(String(value || '-'), 4, y + 3); y += 7.5; };
    ...
    doc.setFillColor(253, 186, 116); doc.rect(0, 80, 54, 6, 'F');
    doc.setTextColor(120, 45, 5); doc.setFontSize(6); doc.text('Hare Krishna \u2022 Serve with devotion', 27, 84, { align: 'center' });
